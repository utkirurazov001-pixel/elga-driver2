// ============================================================
//  KetdikGo Haydovchi — ilova (React Native / Expo)
...
function AppInner() {
    ...
    (async () => {
      ...
      try {
        ...
        if (t && u) {
          ...
          // Token "rolling": har ochilganda jimgina yangilanadi (muddati uzayadi),
          // shuning uchun faol haydovchi hech qachon qayta SMS/parol kiritmaydi.
  ...
  // T-06: token "rolling" yangilash. Ilova ochilganda saqlangan token bilan chaqiriladi;
  // yangi (muddati uzaytirilgan) token qaytadi va saqlanadi — faol haydovchi hech qachon
  // qayta SMS kiritmaydi. Faqat token HAQIQATAN yaroqsiz/muddati o'tган bo'lsa (401)
  // chiqamiz (qayta SMS). Tarmoq xatosida — saqlangan tokenni saqlab qolamiz (oflayn).
  async function refreshToken(existing) {
    try {
      ...
      if (e?.status === 401) { try { await logout(); } catch (_) {} }
    }
  }
  ...
  async function orderAction(action) {
    ...
    try {
      ...
      if (action === 'complete') {
        ...
        setOrder(null);
        setMeter(null); setLiveMeter(null); meterBaseRef.current = null;
        setChatMessages([]);
        loadEarnings();
        updatePersistentNotif('Buyurtma kutilmoqda...');
      ...
      if (e && e.network) {
        ...
        if (action === 'complete') {
          ...
          setOrder(null); setMeter(null); setLiveMeter(null); meterBaseRef.current = null; setChatMessages([]);
        ...
        <View style={s.flex}>
          ...
          {order && (
            <ScrollView
              ...
              <OrderPanel
                order={order} loading={loading} meter={meter} liveMeter={liveMeter}
...
function OrderPanel({ order, loading, meter, liveMeter, onAction, onNavigate, onCall, onChat, onPlayVoice, voiceBusy }) {
